...
const Order = () => {
  ...
  const updateMyOrder = async (id) => {
    try {
      const res = await axios.put(`orders/update-order/${id}`, {
        description: Object(orders?.find((item) => item._id === id))?.description,
      });
      if (res.status === 200) {
  ...
  const payOrder = async (id) => {
    try {
      if (window.confirm(`Pay Order?`) == false) {
        return;
      }
      const res = await axios.put(`orders/pay-order/${id}`, {});
      if (res.status === 200) {
  ...
  return (
    <div>
      ...
      <Header type="list" />
      <div
        style={{ display: "flex", flexDirection: "column", gap: 20, marginTop: 30, marginLeft: 30, marginRight: 30 }}
      >
        {orders?.map((item, index) => {
          ...
          return (
            <div
              className="itemOrder"
              style={{ display: "flex", flexDirection: "row", gap: 20, flexWrap: "wrap" }}
              key={index}
            >
              <div>
                <div>
              ...
              <div style={{ display: "flex", flexDirection: "column", flex: 1, minWidth: "150px" }}>
                ...
                <span
                  style={{
                    color: item.is_active === true ? "blue" : item.is_delete === true ? "red" : "gray",
                    fontWeight: "bold",
                  }}
                >
                  {item.is_active === true
                ...
                <span
                  style={{
                    ...
                    borderRadius: "10px",
                    color: item.is_pay ? "green" : "silver",
                    width: 'fit-content'
                  }}
                >
                  {item.is_pay ? "Đã thanh toán" : "Chưa thanh toán"}
                </span>
              </div>
              <div style={{ display: "flex", flexDirection: "column", flex: 1, minWidth: "150px" }}>
                ...
                />
                <div>{new Date(book_data?.date).toDateString()}</div>
              </div>
              <div style={{ display: "flex", flexDirection: "column", gap: 10, flex: 1, minWidth: "150px" }}>
                ...
                {item.is_active === false && item.is_delete === false && (
                  <button onClick={() => cancelOrder(item._id)}>Huỷ đơn</button>
                )}
                {item.is_pay === false && (
                  <button onClick={() => payOrder(item._id)}>Thanh toán</button>
                )}
              </div>
            </div>
          );
